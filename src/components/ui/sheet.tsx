  ...
  "fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        ...
        bottom:
          "bottom-0 left-0 right-0 mx-auto w-full max-w-lg max-h-[96dvh] rounded-t-2xl border-t data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
...
>(({ side = "right", className, children, hideClose = false, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={cn(sheetVariants({ side }), "z-[60]", className)}
      {...props}
    >
      {hideClose ? null : (
